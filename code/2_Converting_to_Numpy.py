...
I will be pre-processing my training, validation and test images for semantic segmentation using the following steps:
...
TEST_DATASET_PATH = '/user/home/rb21991/DIS/Dissertation/data/Test_data/'
...
# Get list of test patient folders
test_patient_folders = sorted(os.listdir(TEST_DATASET_PATH))

for idx, folder_name in enumerate(test_patient_folders):
    folder_path = os.path.join(TEST_DATASET_PATH, folder_name)
    patient_id = folder_name

    print(f"Processing {patient_id} ({idx + 1}/{len(test_patient_folders)})")
   ...
   # Load and preprocess
    ...
    print(f"✔ Finished and replaced .nii.gz files for test {patient_id}")
