...
'''
def load_img(img_dir, img_list, expected_shape=None):
    images=[]
    for i, image_name in enumerate(img_list):
        if (image_name.split('.')[1] == 'npy'):
            ...
            if expected_shape is not None and image.shape != expected_shape:
                print(f"[SKIP] {image_name}: shape = {image.shape}")
                continue
...
def imageLoader(img_dir, img_list, mask_dir, mask_list, batch_size):
    ...
    while True:
        ...
        while batch_start < L:
            ...
            # Only keep matching shapes
            X = load_img(img_dir, img_list[batch_start:limit], expected_shape=(128, 128, 128, 3))
            Y = load_img(mask_dir, mask_list[batch_start:limit], expected_shape=(128, 128, 128, 4))
...
#train_img_dir = '/user/rb21991/DIS/Dissertation/data/Train_data/Uimages/'
#train_mask_dir = '/user/work/rb21991/DIS/Dissertation/data/Train_data/Umasks/'

# Files to remove
image_file = os.path.join(train_img_dir, 'image_57.npy')
mask_file = os.path.join(train_mask_dir, 'mask_57.npy')

# Delete if exists
for file_path in [image_file, mask_file]:
