#Goal 3_Experiment.py
# Point of this script is to edit 2_experiment.py
# add all modalities and check has around 4 output paths on 1 epoch
# going to remove the seed for now, then add it back later.
#####################################
# run first time removing seed ,then add seed to all the scripts.
# once done split the into 4. 1)PREPROCESSING  2)Convert to Numpy 3) Train Models 4) Evaluate Models on Test Set (but Validation for time)
# if works add change time requested, all modalities, 100 and write up results and discussion using validation set not Test yet.
# remove checks for scripts working 
...
###################################### Checking Script 3 data generator ##########################################
'''
# Get one batch
...
######## checking class weights -might remove or put in later - ##########
'''
import pandas as pd
...
#val_patients   = sorted(os.listdir(val_dir))
'''
########################## adjusted Class weights and loss function ##########################################
...
################################### Script 4: Evaluation and viz ############################################################

#print plots for training and validation loss and accuracy
import matplotlib.pyplot as plt

os.makedirs('results/training_plots', exist_ok=True)  # Create a directory for plots if it doesn't exist
...
# Get the list of validation patients # will need to change this later as will be saved in another script
val_patient_list = val_patients
