# uploading data
# Experiment 1: Goal to add augmentation which hopefully will improve the model performance. MeanIOU.  Add graphs of train and validation loss and accuracy.
# prints out best epoch , training time ,and accuracy . 

# expect visualisations havent been touched yet.
...
#print plots for training and validation loss and accuracy
import matplotlib.pyplot as plt
...
plt.grid(True)
plt.tight_layout()
plt.savefig('training_validation_loss.png')  # 🔽 Save to file
plt.close()
...
plt.grid(True)
plt.tight_layout()
plt.savefig('training_validation_accuracy.png')  # 🔽 Save to file
plt.close()
...
best_iou = 0
best_model_name_iou = None
best_accuracy_for_best_iou = 0
...
for model_file in os.listdir(model_dir):
    ...
    for i in range(num_batches):
        ...
        # Calculate Mean IoU for the current batch
        ...
        batch_iou = iou_metric.result().numpy()
        total_iou += batch_iou
    ...
    # Calculate average IoU and accuracy over all batches
    ...
    if avg_iou > best_iou:
        best_iou = avg_iou
        best_model_name_iou = model_file
        best_accuracy_for_best_iou = avg_accuracy

    if avg_accuracy > best_accuracy:
        best_accuracy = avg_accuracy
        best_model_name_accuracy = model_file
        best_iou_for_best_accuracy = avg_iou

print(f"\nModel with Best Average Mean IoU: {best_model_name_iou} with Average Mean IoU: {best_iou:.4f} and Average Accuracy: {best_accuracy_for_best_iou:.4f}")
print(f"Model with Best Average Accuracy: {best_model_name_accuracy} with Average Accuracy: {best_accuracy:.4f} and Average Mean IoU: {best_iou_for_best_accuracy:.4f}")
...
n_slice = 43  # slice index for visualisation

# Get a random patient
patients = val_patients
random_patient ="UPENN-GBM-00134_11"
patient_path = "../data/Validate_data/UPENN-GBM-00043_11/"
...
for model_file in os.listdir(model_dir):
    ...
    plt.close()
##################################################checking output for MULTIPLE MODELS #######################################################
import os
import numpy as np
import matplotlib.pyplot as plt
from keras.models import load_model
...
# Get the list of validation patients
val_patient_list = val_patients
...
if not val_patient_list:
    ...
    if not os.path.exists(patient_path):
        ...
        if os.path.exists(sample_npy_path):
            ...
            n_slice = 83
            print(f"Visualizing slice {n_slice} for patient 43 NOT {patient_to_visualize}")
